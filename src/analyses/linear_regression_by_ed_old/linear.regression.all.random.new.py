...
trialresponses = pd.read_excel(
    '/Users/charlesconnor/Dropbox/grants/social.memory/selected_cells_time_windowed/spike_count_for_each_trial_windowed.xlsx')
...
affiliationtomatrix = [[0, 19, 9, 38, 84, 27, 13, 14, 4, 9], [15, 0, 9, 41, 4, 13, 19, 71, 12, 0],
...
submissiontomatrix = [[0, 0, 0, 1, 0, 1, 0, 0, 2, 0], [4, 0, 0, 13, 0, 0, 0, 2, 1, 0], [9, 2, 0, 10, 0, 0, 4, 3, 0, 0],
                      [7, 0, 2, 0, 0, 0, 0, 1, 6, 0], [11, 8, 3, 5, 0, 2, 4, 2, 4, 18],
                      [90, 16, 14, 43, 0, 0, 10, 7, 9, 6], [29, 12, 1, 17, 0, 0, 0, 23, 17, 1],
                      [41, 6, 1, 28, 0, 0, 2, 0, 5, 0], [16, 2, 0, 8, 0, 1, 0, 0, 0, 2], [7, 2, 3, 1, 2, 0, 5, 2, 5, 0]]
...
agonismtomatrix = [[0, 4, 2, 7, 4, 7, 3, 9, 3, 2], [0, 0, 0, 2, 1, 1, 8, 4, 2, 0], [0, 1, 0, 0, 0, 7, 0, 1, 0, 2],
...
cellnames = ['9/26 1 2 1', '9/26 3 2 1', '10/3 3 13 1', '10/3 4 10 2', '10/4 1 4 1', '10/4 1 19 3', '10/4 2 18 1',
             '10/4 2 20 2', '10/4 3 2 1', '10/4 3 4 2', '10/4 3 9 1', '10/4 3 9 3', '10/4 4 22 2', '10/4 4 27 1',
             '10/5 1 4', '10/11 1 2', '10/11 3 2', '10/11 3 13', '10/24 2 2', '10/27 4 11', '10/27 4 20', '10/31 1 5',
             '10/31 1 20', '11/8 1 7']
ncells = 24
    ...
    for sourcemonkey in range(0, nmonkeys):
        ...
        for sinkmonkey in range(0, nmonkeys):  # responses don't include subject monkey
            if ((sinkmonkey == sourcemonkey) or (sinkmonkey == subjectmonkey)):
                ...
                response = []
                responsestring = responsestringarray[cells[icell], sinkmonkey + 4 - lostmonkeys]
        ...
        if (Rsquared[ibehavior][sourcemonkey][icell] > 0.25):
            ...
            print('for cell ', icell, 'for source monkey', monkeyname[sourcemonkey], 'Rsquared = ',
                  Rsquared[ibehavior][sourcemonkey][icell])
            # print('y = ', y)
            # print('ypred = ', ypred)
            # print('Ybar = ', sum(y)/len(y))
            # Ybar = sum(y)/len(y)
            # varexp = 0.0
            # vartot = 0.0
            # for ic in range (0, len(y)):
            # print ('ic = ', ic)
            # varexp += (ypred[ic] - Ybar)**2
            # print ('varexp = ', varexp)
            # vartot += (float(y[ic]) - Ybar)**2
            # print ('vartot = ', vartot)
            # print ('varexp = ', varexp)
            # print ('vartot = ', vartot)
            # print ('percent explained variance = ', varexp/vartot)
            ...
            if (nless[ibehavior][sourcemonkey][icell] > 949):
                siglist[icell][ibehavior][sourcemonkey] = nless[ibehavior][sourcemonkey][icell]
                print('for cell ', icell, 'for source monkey', monkeyname[sourcemonkey], 'nless = ',
                      nless[ibehavior][sourcemonkey][icell])
