...
Currently targets SI-sorted units loaded from a pre-stimulus cache
(``sorted_spike_cache_pre1000ms`` by default), so the detectors measure baseline
from the real pre-stimulus window. (Other sources will slot in once their
pre-stim caches exist.)

Two-phase workflow
------------------
1. ``--mode select``  Pick ~N SI-sorted cells (stratified across region/session
   from the pre-stim cache), write ``candidate_cells.csv`` +
   ``ground_truth_template.xlsx``, and render an UNMARKED raster+PSTH (with the
   pre-stim baseline shown) per cell into ``rasters_for_annotation/`` for you to
   inspect and fill in the windows you actually see.
...
2. ``--mode score``   Run every detector on each cell, render a comparison figure
   ...
   ``scores_per_cell.csv``, ``scoreboard.csv`` and ``scoreboard.png``.

``--mode score`` also runs without a filled template (overlays only, no scoring).

Needs the lab machine's pre-stim cache. Verify the code with
...
# --------------------------------------------------------------------------- #
def run_select(out_dir: str = OUT_DIR, *, n: int = 20, seed: int = 0, xlim: float = 2.4,
               cache_subdir: str = DEFAULT_CACHE_SUBDIR, pre_stim: float = DEFAULT_PRE_STIM,
    ...
    for (sk, csub, date, rnd), rows in _sessions(cands).items():
        ...
        for row in rows:
            ...
            if unit.empty:
                ...
                continue
            plot_cell_raster(
                unit, title=f"{row['Source']} · {row['NeuronID']}  ({row.get('Region','?')})",
                xlim=xlim, pre_stimulus_time=pre,
    ...
    for (sk, csub, date, rnd), rows in _sessions(cands).items():
        ...
        for row in rows:
            ...
            for d in detectors:
                ...
                except Exception as e:  # e.g. a baseline detector on a no-pre-stim cell
                    print(f"[score] {d.name} failed on {key}: {e}")
            results_by_cell[key] = {name: r.windows for name, r in results.items()}
            plot_cell_with_windows(
                unit, detectors, results, truth_windows=truth.get(key),
                title=f"{row['Source']} · {row['NeuronID']}  ({row.get('Region','?')}/{row.get('UnitType','?')})",
                bin_s=bin_s, xlim=xlim, pre_stimulus_time=pre,
...
def _cli(argv=None):
    ...
    p.add_argument("--cache-subdir", default=DEFAULT_CACHE_SUBDIR)
    ...
    if args.mode == "select":
        run_select(args.out, n=args.n, seed=args.seed, xlim=args.xlim,
                   cache_subdir=args.cache_subdir, pre_stim=args.pre_stim)
    else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        N_CELLS = 20                                 # select: number of cells
        SEED = 0
        CACHE_SUBDIR = DEFAULT_CACHE_SUBDIR          # SI-sorted pre-stim cache
        ...
        if MODE == "select":
            run_select(OUT_DIR, n=N_CELLS, seed=SEED, xlim=XLIM_S,
                       cache_subdir=CACHE_SUBDIR, pre_stim=PRE_STIM, sessions=SESSIONS)
        else:
