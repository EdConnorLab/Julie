...
"""
Run social encoding analysis across feature modes and groups.

Grid: 4 feature_modes × 3 groups (Zombies, Best Frans, Instigators)
       (to_subject and from_subject skip non-home groups automatically)
Region: AMG
Window: 200–600 ms

Produces a summary table comparing marginal R² and permutation p-values.
"""
...
def run_sweep():
    ...
    cfg = SocialEncodingConfig(
        ...
        n_permutations=5000,
        save_plots=True,
        save_dir='social_encoding_sweep',
    )
    cfg.validate()
    ...
    for mode in feature_modes:
        ...
        for group in groups:
            ...
            # PCA scatter plot
            save_dir_pca = f"{cfg.save_dir}/{cfg.region}/pca_plots"
    ...
    print(f"\n\n{'='*90}")
    print(f"SWEEP SUMMARY — AMG, 200–600 ms, {cfg.n_permutations} permutations")
    print(f"{'='*90}")
    ...
    save_dir = f"{cfg.save_dir}/{cfg.region}"
    os.makedirs(save_dir, exist_ok=True)

    plot_sweep_comparison(results_df, cfg, save_path=f"{save_dir}/sweep_comparison.png")
    plot_sweep_perm_pvalues(results_df, cfg, save_path=f"{save_dir}/sweep_perm_pvalues.png")

    # Save results table
    results_df.to_csv(f"{save_dir}/sweep_results.csv", index=False)
...
def plot_sweep_comparison(results_df, cfg, save_path=None):
    ...
    ax.set_title(f"Social Encoding: AMG, 200–600 ms\n"
...
def plot_sweep_perm_pvalues(results_df, cfg, save_path=None):
    ...
    ax.set_title('Permutation p-values across conditions', fontsize=12)
...
def run_pcs_sweep(region='ALL', max_pcs=5, n_permutations=5000):
    """Sweep number of PCs (1 to max_pcs) for full_profile mode.
    ...
    print(f"\n{'='*70}")
    print(f"PCs Sweep — full_profile, {region}, 200–600 ms")
    print(f"  PCs: 1 to {max_pcs}")
    ...
    # ── Summary ──
    print(f"\n\n{'='*70}")
    print(f"PCs SWEEP SUMMARY — full_profile, {region}")
    print(f"{'='*70}")
    ...
    # ── Plot ──
    save_dir = f"{cfg.save_dir}/{region}"
    os.makedirs(save_dir, exist_ok=True)

    plot_pcs_sweep(results_df, cfg, save_path=f"{save_dir}/pcs_sweep.png")

    results_df.to_csv(f"{save_dir}/pcs_sweep_results.csv", index=False)
    print(f"\nResults saved to {save_dir}/")
...
def plot_pcs_sweep(results_df, cfg, save_path=None):
    ...
    fig.suptitle('full_profile PCs sweep', fontsize=13, y=1.02)
    fig.tight_layout()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'pcs':
        # python run_social_encoding_sweep.py pcs [region] [max_pcs]
        region = sys.argv[2] if len(sys.argv) > 2 else 'ALL'
        max_pcs = int(sys.argv[3]) if len(sys.argv) > 3 else 5
        run_pcs_sweep(region=region, max_pcs=max_pcs)
    else:
        run_sweep()
