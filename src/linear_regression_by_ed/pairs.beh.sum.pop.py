...
for i in range (0, nmonkeys):
    max_list = []	#outer or 1st dimension of list array is max monkey
    for j in range (0, nmonkeys):
        max_list.append(0.0)
    pair_dif.append(max_list)
...
for i in range (0, nmonkeys):
    ...
    for j in range (0, nmonkeys):
        max_list.append(0)
    anon_pair_less.append(max_list)
...
for icell in range (0, ncells):
    ...
    obs_pair_dif = (pair_avg - pop_avg) / maxmean
    sum_obs += obs_pair_dif
    means[idx_max] = maxmean
    means[idx_nxt] = nxtmean
    pair_dif[idx_max][idx_nxt] += obs_pair_dif
...
for ir in range (0, 1000):
    ...
    for icell in range (0, ncells):
        ...
        pair_avg = (maxmean + nxtmean) / 2.0
        pop_avg = sum(means) / (len(means) - 2.0)
        rnd_pair_dif = (pair_avg - pop_avg) / maxmean
        sum_rnd_obs += rnd_pair_dif
        means[idx_max] = maxmean
        means[idx_nxt] = nxtmean
        rnd_dif[idx_max][idx_nxt] += rnd_pair_dif
        ...
        if (idx_nxt < subjectmonkey):
            ...
            nxtmonkey = idx_nxt +1
        rndaff = affiliationtomatrix[maxmonkey][nxtmonkey] + affiliationfrommatrix[maxmonkey][nxtmonkey]
        rndaff_prd = aff * obs_pair_dif
        rnd_aff_sum += aff
        rnd_aff_prd_sum += aff_prd
        
    if (rnd_aff_sum < aff_sum):
        nless_aff_sum += 1
    ...
    for i in range (0, nmonkeys):
        for j in range (0, nmonkeys):
            if (rnd_dif[i][j] < pair_dif[i][j]):
                pair_less[i][j] += 1
                
                
            for k in range (0, nmonkeys):
                for l in range (0, nmonkeys):
                    if (rnd_dif[i][j] < pair_dif[k][l]):
                        anon_pair_less[k][l] += 1
                        
    if (sum_rnd_obs < sum_obs):
        sum_less += 1
        
    #print('sum_rnd_obs = ', sum_rnd_obs, 'sum_obs = ', sum_obs)
        
print ('nless_aff_sum = ', nless_aff_sum)
print ('nless_aff_prd_sum = ', nless_aff_prd_sum)
...
for i in range (0, nmonkeys):
    for j in range (0, nmonkeys):
        print ('for ', monkeyname_notsubject[i], 'and', monkeyname_notsubject[j])
        print ('pair_less:', pair_less[i][j], 'anon_pair_less:', anon_pair_less[i][j])
